...
本设计文档沉淀 2026-06-09 至 2026-06-10 已确认的头脑风暴结论，并吸收用户后续 review：好感度变化不应每轮都发生，也不应只有 `-3..+3` 一个全局上限。V1 应建立“对话事件 -> 安全校验 -> 游戏效果”的框架：普通闲聊通常不改变好感度；只有出现明确情绪变化、利益相关、送礼、辱骂、诋毁、攻击、威胁等有意义事件时，才按分档规则改变好感度。

## 目标

- 玩家与 NPC 对话后，系统可以识别“有游戏意义的对话事件”，并在必要时修改游戏内 NPC 好感度。
- 普通闲聊、问候、无明显立场变化的信息交换默认不改变好感度。
- AI 只负责提出“效果建议”，不能直接执行游戏命令。
- Mod 负责解析、分类、校验、限幅、执行游戏侧好感度变化，并给玩家显示简短提示。
- 好感度变化按事件类型和强度分档，不再使用 `-3..+3` 作为唯一上下界。
- 关系变化原因要写入 NPC 记忆，影响后续对话语气和关系阶段。
- 为后续“通过对话触发具体游戏行为”预留统一事件框架，但 V1 只执行好感度变化。
...
- 第一版仍然同步修改游戏内关系/好感度。
- 游戏内仍显示简短提示，但只在真实好感度变化成功后显示。
- 原先的 `-3..+3` 不再是全局唯一上限，改为“轻微/中等/重大/严重”分档。
- 普通对话默认 `favor_delta = 0`。
- 对话效果写入游戏内 NPC 好感度，执行接口使用 `NPCEx.GetFavor` 和 `NPCEx.AddFavor`。
- 通过聊天获得的正向好感度最高到 `60`；当前好感度达到或超过 `60` 后，正向聊天增益归零。负向对话效果仍可降低好感度。
- 辱骂、诋毁、攻击、威胁、背叛等场景可以大幅降低好感度。
- 送礼、让利、救助、完成承诺等场景可以较大幅增加好感度。
...
5. Mod 解析隐藏效果 JSON，得到建议事件类型、强度和好感度变化。
6. Mod 判断本轮是否属于可触发事件。
7. Mod 根据事件类型、强度、NPC 上下文和配置表校验最终好感度变化。
8. 校验通过后，Mod 调用游戏内好感度接口修改 NPC 好感度。
...
- `impact_level`：事件强度分档。
- `favor_delta`：AI 建议的好感度变化，整数；最终执行值由 Mod 校验后决定。
...
| --- | --- | --- |
| `none` | 普通闲聊、问候、无明确立场变化 | 不改好感度 |
| `emotional_shift` | 明确表达尊重、歉意、失望、冒犯等情绪变化 | 按强度小到中幅变化 |
| `interest_gain` | 玩家让利、帮忙、承诺有益事项、完成对 NPC 有利行为 | 增加好感度 |
| `interest_loss` | 玩家损害 NPC 利益、索取过度、违背承诺、制造麻烦 | 降低好感度 |
| `gift` | 送礼、赠药、赠法宝、提供资源 | 按礼物价值增加好感度 |
| `insult_attack` | 辱骂、诋毁、威胁、羞辱、攻击名誉或立场 | 中到大幅降低好感度 |
| `trust_break` | 欺骗、背叛、泄密、出卖利益 | 大幅降低好感度 |
...
- 正向 `favor_delta` 执行前必须读取当前好感度；如果当前好感度已达到或超过 `60`，最终增益为 `0`。
- 正向 `favor_delta` 会被裁剪到不超过 `60`，例如当前好感度 `58` 且建议 `+8`，最终只执行 `+2`。
- 负向 `favor_delta` 不受 `60` 上限影响，辱骂、威胁、背叛等仍可降低好感度。
- 同一玩家消息只能处理一次效果，避免重试导致重复加减。
- NPC ID 不明确、当前对话对象不明确或游戏状态不可写：不执行好感度变化。
...
- 正向好感度变化执行前必须套用聊天收益上限：当前好感度 `>= 60` 时归零，执行后不得超过 `60`。
- 负向好感度变化不受聊天收益上限影响。
...
- AI 没有输出效果块：显示正文，不改好感度。
- 效果块格式错误：显示正文，不改好感度，记录一次解析失败。
- AI 输出普通对话：显示正文，不改好感度。
- 找不到当前 NPC ID：显示正文，不改好感度。
- 游戏好感度接口调用失败：显示正文，不显示成功提示，记录失败原因。
- 记忆写入失败：不回滚已成功的游戏好感度变化，但记录失败。
- AI 输出危险触发器：忽略触发器，只处理合规的好感度变化。
...
- 选择普通 NPC 测试以下场景：
  ...
  - 送礼或让利：中到大幅增加。
  - 辱骂、诋毁、威胁：中到大幅降低。
  - 严重背叛候选：只记录好感度和候选，不触发战斗。
- 检查提示是否只在真实修改成功后出现。
- 检查重复发送或 AI 重试不会重复加减同一轮好感度。
- 检查 NPC 记忆是否记录事件类型、强度和关系变化。

## 风险

- 游戏侧好感度接口签名、调用时机或副作用尚未确认。
...
- 游戏内最安全的好感度修改接口是哪一个：`NPCEx.AddFavor`、已有 favor patch，还是其他内部管理器。
- 当前 AI 回复链路中最适合插入 parser 的位置在哪里。
- 游戏提示应复用现有 UI 通知，还是先写入聊天面板系统提示。
- 礼物价值如何从游戏数据读取，是否能区分普通物品和贵重资源。
- `major` 与 `severe` 的默认范围是否需要根据游戏原始好感度系统尺度再收紧。
